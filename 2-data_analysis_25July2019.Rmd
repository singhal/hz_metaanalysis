...
coef_report <- function(models) {
  ...
  coefm$coefficient = paste(coefm$coefficient, coefm$stderr, sep="±")
...
a = ggplot(s, aes(critter)) + geom_bar() +  
     ...
     theme(axis.text.y=element_blank(), 
           axis.ticks.y=element_blank()) + ylim(0, max(spcts + 6))
...
ab = plot_grid(a, map, labels = c("A", "B"), nrow = 1, align = "h", rel_widths = c(0.45, 0.55))
...
s$log_width = scale(log(s$gm_mean_width))
...
plt1 = ggplot(s, aes(log_dispersal_ind, log_width)) + 
...
plt3 = ggplot(s, aes(log_dist, log_width)) + 
  geom_point(pch=16, color="gray") + 
  xlab("log(mtDNA distance)") + ylab("") + 
  geom_abline(intercept = nucDNA_model_coefs["(Intercept)", "Estimate"], slope = nucDNA_model_coefs["log_mean_dist", "Estimate"]) +
  geom_abline(intercept = mtDNA_model_coefs["(Intercept)", "Estimate"], slope = mtDNA_model_coefs["log_mean_dist", "Estimate"], linetype = 2)
...
save_plot(paste(outdir, 'mean_data.png', sep=""), prow,
          nrow = 1, ncol = 3, base_width = 2.5, base_height = 2.25)
```
...
get_coef2 <- function(x) {
  ...
  coeft$variable = rownames(coeft)
  rownames(coeft) = NULL
  coeft$coefficient = as.numeric(gsub("±\\S+", "", coeft$coefficient))
  return(coeft)
}
...
coefs$ymin = coefs$xmin * coefs$coefficient + coefs$intercept
coefs$ymax = coefs$xmax * coefs$coefficient + coefs$intercept

a = ggplot(xx, aes(log_dispersal_only_ind, log_gm_mean_width)) + 
...
b = ggplot(xx, aes(log_mean_dist, log_gm_mean_width)) + 
    geom_point(alpha=0.5, size = 1) + facet_wrap(~critter, nrow = 1) + 
